...
class TransactionDB {
  ...
  Future<List<Transactions>> loadAllData() async {
    ...
    for (var record in snapshot) {
      ...
      String dtype = record['dtype'].toString();
      String date = record['date'].toString();
      // print(record['title']);
      transactionList.add(Transactions(
          id: id, title: title, amount: amount, dtype: dtype, date: date));
    }
    db.close();
  ...
  Future deleteData(Transactions statement) async {
    ...
    //filter with 'id'
    final finder = Finder(filter: Filter.equals('date', statement.date));
  ...
  Future<Transactions?> loadSingleRow(int rowId) async {
    ...
    String dtype = snapshot.first['dtype'].toString();
